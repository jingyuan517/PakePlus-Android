document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (window.NativeStorage && NativeStorage.isNative) {
            NativeStorage.restore().then(function (backup) {
                if (backup && Array.isArray(backup.messages) && backup.messages.length > 0
                    && Array.isArray(messages) && backup.messages.length > messages.length) {
                    console.warn('[boot] 从原生文件恢复备份 (' + backup.messages.length + ' 条)');
        ...
        setInterval(() => {
            ...
            if (cs) {
                var cfg = cs.getStatus ? cs.getStatus() : { connected: false };
                if (cfg.connected) {
                    cs.upload().catch(function () {});
                }
            }
        }, 3 * 60 * 1000);
